...
import requests
from dagster import asset, AssetExecutionContext, MetadataValue, MaterializeResult, AutoMaterializePolicy # import the `dagster` library
...
def topstory_ids(context: AssetExecutionContext) -> None:
...
@asset(deps=[topstory_ids_2], group_name="pipeline_2")
def topstories_2(context: AssetExecutionContext) -> pd.DataFrame:
    with open("data/topstory_ids_2.json", "r") as f:
        topstory_ids = json.load(f)
...
@asset(group_name="pipeline_2", auto_materialize_policy=AutoMaterializePolicy.eager())
def most_frequent_words_2(context: AssetExecutionContext, topstories_2) -> MaterializeResult:
    stopwords = ["a", "the", "an", "of", "to", "in", "for", "and", "with", "on", "is"]

    #topstories = pd.read_csv("data/topstories.csv")

    context.log.info(f"Recieved data  : {len(topstories_2)} rows!")

    # loop through the titles and count the frequency of each word
    word_counts = {}
    for raw_title in topstories_2["title"]:
        title = raw_title.lower()
        for word in title.split():
            cleaned_word = word.strip(".,-!?:;()[]'\"-")
            if cleaned_word not in stopwords and len(cleaned_word) > 0:
